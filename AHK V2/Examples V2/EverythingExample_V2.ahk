...
draw(){
	...
	if (overlay.BeginDraw()) {
		...
		if (step = 1) {
			...
			if (objs.Length < 500)
				Loop 1
					objs.push(rain(-500,overlay.width+500,-100,0,(random(200,255)<<24)+(random(95,230)<<16)+(random(200,250)<<8)+random(222,255),overlay.height))
			for k,v in objs
		...
		if (overlay.GetMousePos(&mx,&my)) {
			if (lastX != -1 and (lastX != mx or lastY != my)) {
				trails.push(mouseTrail(lastX,lastY,mx,my,trailColor))
			}
			lastX := mx
			lastY := my
		} else {
